...
import com.jmjbrothers.student_management_system.repository.MapCustomUserRepository;
...
public class SecurityConfig {
	...
	/*
	 * @Bean public UserDetailsService userDetailsService() { // Regular user with
	 * basic access UserDetails regularUser =
	 * User.builder().username("user").password(passwordEncoder().encode("user123"))
	 * // .roles("REGULAR_USER") .authorities("ROLE_REGULAR_USER",
	 * "SENIOR").build();
	 * 
	 * System.out.println(regularUser.getAuthorities());
	 * 
	 * UserDetails hemel =
	 * User.builder().username("hemel").password(passwordEncoder().encode("hemel123"
	 * )) // .roles("REGULAR_USER") .authorities("ROLE_REGULAR_USER",
	 * "JUNIOR").build();
	 * 
	 * System.out.println(hemel.getAuthorities());
	 * 
	 * // Manager with intermediate access String manager123 =
	 * passwordEncoder().encode("manager123"); System.out.println(manager123);
	 * UserDetails manager =
	 * User.builder().username("manager").password(manager123).roles("MANAGER").
	 * build();
	 * 
	 * System.out.println(manager.getAuthorities());
	 * 
	 * // Admin with full access UserDetails admin =
